...
pnpm dlx wrangler deploy --config ./wrangler.toml
```
...
3. 选择本仓库后，在 Build / Deploy 设置中填：
   - Build command: `pnpm dlx opennextjs-cloudflare build`
   - Deploy command: `npx wrangler deploy --config ./wrangler.toml`
4. 在仓库根目录直接使用 `wrangler.toml`，按需修改 `name`（建议全局唯一）和 `DEFAULT_MODEL`。

> 为什么 deploy 命令要加 `--config ./wrangler.toml`：可以避免 Wrangler 在 pnpm workspace 根目录下出现自动检测报错。
...
并确保 `wrangler.toml` 在仓库根目录且 `main` 指向 `.open-next/worker.js`。
